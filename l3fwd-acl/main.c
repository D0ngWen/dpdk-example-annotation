...
// 函数功能：把in中字符串转化并放入放入fd中，in指向下一个字符串
// 输入参数：
//		in: char *in[]
//		fd: 提取的数字
// 		base: 输入字符串进制类型,
//			如10,16,0表示十进制，16进制，自动选择
//		lim: 转化后的数字限制
//		dlm: delimit，分隔符
// strtuul()函数说明：val = strtoul((in), &end, (base));
//		end指向第一个非法字符（不是数字）
...
// 解析一行ACL-5元组规则到对应的rte_acl_rule结构体中
// has_userdata表明是否有路由选项，路有选项表明转发的port_id
static int
		...
		/*    parser -> parse_cb_ipv4vlan_rule */
{
	...
	while (fgets(buff, LINE_MAX, fh) != NULL) {
		...
		if (s == ROUTE_LEAD_CHAR) {
			...
			// route_acl中队转发的port_id+FWD_PORT_SHIFT
...
dump_acl_config(void)
{
	...
	// 表示选择DPDK-ACL库中标量分类方法，不需要任何硬件支持
	// DPDK-ACL库中分类方法有：
