...
# $()和 ``(反引号)作用一样，都是用来进行命令替换
# cut命令 -d：指定字段的分隔符，默认的字段分隔符为“TAB”；-f：显示指定字段的内容；
# tr命令 -d 删除制定字符，tr -d ' '删除所有空格
...
setup_target()
{
	...
	echo "$PWD :$RTE_TARGET"
	exit 0
	# ${RTE_TARGET##*-} 表示 忽略RTE_TARGET字符最后一个'-'号，及其左边的内容
	compiler=${RTE_TARGET##*-}
	if [ "$compiler" == "icc" ] ; then
		# ${RTE_TARGET%%*-} 表示 忽略RTE_TARGET字符最后一个'-'号，及其右边的内容
		platform=${RTE_TARGET%%-*}
...
remove_mnt_huge()
{
	...
	# /dev/null 被称为位桶(bit bucket), 当你读它的时候，它会提供无限的空字符(NULL, ASCII NUL, 0x00)。
	# 重定向到/dev/null，不返回输出
	# grep -s 不输出错误
	grep -s '/mnt/huge' /proc/mounts > /dev/null
	if [ $? -eq 0 ] ; then
